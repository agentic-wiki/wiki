...
- **Entries are relative to `wiki.toml` (the bundle root), and the list is unified** — one `ignore`, two effects:
  ...
  - a path resolving **outside** the bundle (e.g. `../PRD.md`) → any out-of-bundle link whose target resolves to it has its `out-of-bundle link -> …` advisory suppressed (see [out-of-bundle links](./004-out-of-bundle-links.md)).
