...
v1 embeds the starter in this repo (`internal/scaffold/files/`). Sourcing it from the separate `agentic-wiki/template` repo as a go-module dependency, plus `--template`/`--from`/`skill install`, is the [scaffold registry](/3-graph-and-mutation/005-scaffold-registry.md).
