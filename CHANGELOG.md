...
- **`wiki check` flags malformed hybrid links.** A `[[text](/x.md)]` (Obsidian-style outer brackets wrapped around a standard markdown link) is now a warning. It used to pass silently: the wikilink parser needs a closing `]]` (absent here), while the markdown parser matches the *inner* `[…](…)` and resolves its target, so nothing reported it broken. The only trace is a stray `[` the parser swallows into the link text, which is exactly the signal the check keys on. Warning severity, so exit stays `0`. Narrow by design (a well-formed link never trips it); external-target hybrids, the trailing-only `[text](/x.md)]` typo, and autofix are out of scope.
