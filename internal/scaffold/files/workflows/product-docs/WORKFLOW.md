...
**Types are not a first-run question.** The shipped set (`concept`, `reference`, `guide`, `example`, plus `note`/`decision`/`source`) covers the start; add one to `wiki.toml` when a real need appears (a `model` type once you document many data entities, `tutorial` vs `howto` if you follow Diátaxis), and fold `reference` into `concept` if that split never earns its keep. Grow the vocabulary on demand rather than interrogating the user up front.
