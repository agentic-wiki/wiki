...
Entries link with standard Markdown, root-absolute from the bundle root: `[Income](/finance/income.md)`. No `[[wikilinks]]`. Two reserved filenames carry no `type`: `index.md` (a folder's navigation surface) and the optional `log.md` (a dated chronicle). Files listed in `wiki.toml`'s `skip` list (this manual, `WORKFLOW.md`) are operating docs, not wiki entries.
